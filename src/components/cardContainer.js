...
const CardContainer = ({ incidents }) => {
	...
	const getTotalNumberOfIncidentsOpen = (incidents = []) => {
		return incidents.filter(incident => incident.incidentStatusId !== "RESOLVED")
			.length;
	};
	...
	const getAverageResolutionTime = (incidents = []) => {
		const beforeAverage = incidents
			.filter(
				incident =>
					incident.incidentStatusId === "RESOLVED" && !isNaN(incident.duration),
			)
			.reduce((accum, current) => accum + current.duration, 0);
		return secondsToDhms(beforeAverage / incidents.length);
	};
	return (
					...
					<Grid item>
						<Card className={classes.cardRoot}>
							<CardContent>
								...
								<Divider />
								<Typography>{getNumberOfIncidentsPerDays(incidents)}</Typography>
							</CardContent>
						</Card>
					</Grid>
					<Grid item>
